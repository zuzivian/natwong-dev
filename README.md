...
1. **Ship a live, real MVP within a single ~1-hour focused session** — validated: site was scoped, built, and deployed inside the time-boxed window.
...
4. **Produce something in-house I built myself**, not a template, to serve as an early entry in a broader portfolio-building strategy ahead of GSB recruiting — validated: custom-built single-page site with an original visual system (no template).
...
- **Case studies / project write-ups** — deferred, intentionally. Build-log section shipped as an explicit placeholder ("first entries in progress") rather than faked content, because the actual build projects (part of the pre-matriculation portfolio strategy) don't exist yet.
...
- As a **recruiter or networking contact** who just met me, I want to quickly understand what makes my background distinctive, so that I remember me after a 15-minute coffee chat.
...
- As **me**, I want the deploy process to require zero ongoing manual work, so that updating the site later doesn't compete with higher-priority pre-matriculation tasks.
...
- [ ] Case studies for build projects, once they exist (planned for the Aug 16–Sep 8 travel window, per the broader portfolio strategy)
...
**Lagging indicators** (only measurable over the recruiting cycle, not yet evaluated):
...
- Whether it gets referenced positively during actual PM recruiting conversations post–Oct 29 networking window
...
- **Design/content**: Should the build-log placeholder be replaced with real entries as soon as the first post-matriculation project ships, or batched until 2–3 are ready? *(owner: Nat)*
- **Content**: Does the "systems thinker across domains" narrative need updating once the functional PM lane (consumer / TPM / infra-platform) is decided, or does it stay evergreen across that decision? *(owner: Nat)*
...
- **Built and shipped**: single focused session, ahead of the Aug 15 end of the Singapore pre-matriculation window
- **Dependency**: build-log/case-study content depends on portfolio projects planned for the Aug 16–Sep 8 travel window (secondary priority behind placement exam prep during that window, per existing task sequencing)
- **No hard external deadline** — unlike GSB administrative tasks, this has no imposed cutoff, which is precisely why it was scoped tightly to avoid open-ended time investment during an already dense pre-matriculation period
